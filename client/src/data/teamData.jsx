const teamData = {
  leadership: [
    {
      id: 1,
      name: "Aviral",
      role: "Founder & CEO",
      image: "https://via.placeholder.com/300x400?text=CEO",
      bio: "Former Google engineer with 15+ years of experience building scalable solutions. Founded Codespie with a vision to make quality software accessible to businesses of all sizes.",
      socialLinks: {
        linkedin: "https://linkedin.com/",
        twitter: "https://twitter.com/",
        github: "https://github.com/",
      },
    },
    {
      id: 2,
      name: "Nouman Khan",
      role: "Co-Founder & CTO",
      image: "https://via.placeholder.com/300x400?text=CTO",
      bio: "Full-stack architect specialized in cloud infrastructure and DevOps. Passionate about building reliable systems that scale with your business needs.",
      socialLinks: {
        linkedin: "https://linkedin.com/",
        twitter: "https://twitter.com/",
        github: "https://github.com/",
      },
    },
  ],
  team: [
    {
      id: 3,
      name: "Sajal",
      role: "Lead Full-stack Developer",
      image: "https://via.placeholder.com/300x400?text=Frontend",
      bio: "React specialist with an eye for detail. Creates beautiful, responsive interfaces that users love.",
      socialLinks: {
        linkedin: "https://linkedin.com/",
        github: "https://github.com/",
      },
    },
    {
      id: 4,
      name: "Mahek Garg",
      role: "Full Stack Developer",
      image: "https://via.placeholder.com/300x400?text=Backend",
      bio: "Node.js expert focused on building robust APIs and microservices. 8+ years in system architecture.",
      socialLinks: {
        linkedin: "https://linkedin.com/",
        github: "https://github.com/",
      },
    },
    {
      id: 5,
      name: "David Kim",
      role: "Full Stack Developer",
      image: "https://via.placeholder.com/300x400?text=FullStack",
      bio: "Jack of all trades who thrives in the full development lifecycle. Specializes in React and Express.",
      socialLinks: {
        linkedin: "https://linkedin.com/",
        github: "https://github.com/",
      },
    },
    {
      id: 6,
      name: "Olivia Taylor",
      role: "DevOps Engineer",
      image: "https://via.placeholder.com/300x400?text=DevOps",
      bio: "Automation enthusiast who keeps our CI/CD pipelines flowing smoothly. AWS certified expert.",
      socialLinks: {
        linkedin: "https://linkedin.com/",
        github: "https://github.com/",
      },
    },
    {
      id: 7,
      name: "James Wilson",
      role: "UI/UX Design Lead",
      image: "https://via.placeholder.com/300x400?text=Design",
      bio: "Award-winning designer focused on creating intuitive user experiences that balance form and function.",
      socialLinks: {
        linkedin: "https://linkedin.com/",
        dribbble: "https://dribbble.com/",
        behance: "https://www.behance.net/",
      },
    },
    {
      id: 8,
      name: "Aisha Mohammed",
      role: "Visual Designer",
      image: "https://via.placeholder.com/300x400?text=Visual",
      bio: "Passionate about creating brand identities and visual systems that communicate clearly and beautifully.",
      socialLinks: {
        linkedin: "https://linkedin.com/",
        dribbble: "https://dribbble.com/",
      },
    },
    {
      id: 9,
      name: "Ryan Cooper",
      role: "Marketing Director",
      image: "https://via.placeholder.com/300x400?text=Marketing",
      bio: "Data-driven marketer with experience in both B2B and B2C environments. Focuses on growth strategies that deliver measurable results.",
      socialLinks: {
        linkedin: "https://linkedin.com/",
        twitter: "https://twitter.com/",
      },
    },
    {
      id: 10,
      name: "Emma Lewis",
      role: "Content Strategist",
      image: "https://via.placeholder.com/300x400?text=Content",
      bio: "Former journalist who crafts compelling narratives that engage audiences and drive conversions.",
      socialLinks: {
        linkedin: "https://linkedin.com/",
        twitter: "https://twitter.com/",
      },
    },
  ],
};

const teamMembers = [
  {
    name: "Aviral",
    role: "Founder",
    image: "/api/placeholder/400/400",
    linkedin: "https://linkedin.com/in/aviral",
    email: "aviral@company.com",
  },
  {
    name: "Nouman",
    role: "Co-Founder",
    image: "/api/placeholder/400/400",
    linkedin: "https://linkedin.com/in/nouman",
    email: "nouman@company.com",
  },
  {
    name: "Sajal",
    role: "Full-Stack Developer",
    image: "/api/placeholder/400/400",
    linkedin: "https://www.linkedin.com/in/sajalgoel349/",
    email: "sajal@company.com",
  },
  {
    name: "Mahek",
    role: "Full-Stack Developer",
    image: "/api/placeholder/400/400",
    linkedin: "https://www.linkedin.com/in/mahek-garg-81ab68299/",
    email: "mahek@company.com",
  },
];

export { teamData, teamMembers };
