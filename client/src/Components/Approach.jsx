    ...
    <section className="py-24 bg-gradient-to-b from-gray-50 to-white">
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div className="hidden md:block relative mb-12">
          <div className="absolute top-1/2 left-0 right-0 h-1 bg-gradient-to-r from-purple-500 via-blue-500 to-green-500 transform -translate-y-1/2"></div>
          <div className="flex justify-between">
            {steps.map((step, index) => (
              <div key={index} className="relative">
                <div
                  className={`w-8 h-8 rounded-full ${step.bg} border-4 ${step.borderColor} z-10 relative`}
                ></div>
              </div>
            ))}
          </div>
        ...
        <div className="grid md:grid-cols-4 sm:grid-cols-2 gap-6 lg:gap-8">
          {steps.map((step, index) => (
            <div
              key={index}
              className={`group relative bg-white p-6 border border-gray-200 rounded-xl shadow-lg ${step.shadowColor} transition-all duration-300 hover:shadow-xl hover:-translate-y-1 ${step.hoverBorder}`}
            >
              <div className="absolute -top-4 -right-4 w-12 h-12 rounded-full bg-white shadow-md flex items-center justify-center border-2 border-gray-100 font-bold text-lg text-purple-800">
                {step.number}
              </div>

              <div
                className={`${step.bg} ${step.iconColor} ${step.hoverBg} p-4 rounded-full mb-6 transition-colors duration-300 w-16 h-16 flex items-center justify-center`}
              >
                {step.icon}
              </div>
